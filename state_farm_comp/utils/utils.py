...
class MixIterator(object):
    ...
    def copy_elements(source_directory=os.getcwd(), dest_d = os.getcwd(), n=20, action="copy"):
        ...
        for d in glob('*/'):
            ...
            for i in range(n):
                if action == "copy":
                    copyfile(shuf[i], dest_d + '/' + shuf[i])
                elif action=="move":
                    os.rename(shuf[i], dest_d + '/' + shuf[i])
                else:
                    print('Error, action variable improperly defined')
